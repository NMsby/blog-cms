    ...
    <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="max-w-2xl">
        ...
        <div class="bg-white shadow-sm rounded-lg p-6">
            <div class="mb-4">
                ...
                <x-text-input id="name" type="text" name="name" :value="old('name', $category->name)" class="block mt-1 w-full" required />
            ...
            <div class="mb-4">
                ...
                <x-text-input id="meta_title" type="text" name="meta_title" :value="old('meta_title', $category->meta_title)" class="block mt-1 w-full" />
                <x-input-error :messages="$errors->get('meta_title')" class="mt-2" />
            </div>

            <div class="mb-4">
                <x-input-label for="meta_description" value="Meta Description" />
                <textarea id="meta_description" name="meta_description" rows="2" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('meta_description', $category->meta_description) }}</textarea>
                <x-input-error :messages="$errors->get('meta_description')" class="mt-2" />
            </div>

            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="is_visible" value="1" {{ old('is_visible', $category->is_visible) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
